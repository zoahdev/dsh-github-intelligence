#!/usr/bin/env node
/**
 * Real-network smoke for every tool added in v2.3.0 + v2.4.0 + v2.5.0
 * (30 tools). Uses the public APIs of GitHub, GitLab, Gitee, npm, Stack
 * Exchange, Reddit, dev.to, RubyGems, and NuGet (optionally authenticated
 * GitHub via GH_TOKEN).
 *
...
async function fetcher(path, signal) {
  const headers = { 'User-Agent': 'dsh-github-intelligence/2.5.0-smoke', Accept: 'application/vnd.github+json' }
...
async function fetcherRaw(path, signal) {
  const response = await fetch(path, {
    headers: { 'User-Agent': 'dsh-github-intelligence/2.5.0-smoke', Accept: 'application/json' },
    signal,
  })
  if (!response.ok) {
...
for (const [name, args, fetchFn] of cases) {
  ...
  try {
    ...
    if (name.startsWith('reddit_')) {
      skipped += 1
      console.log(`SKIP ${name}: Reddit blocks this network (${message.slice(0, 80)}); verified via unit fixture, works from residential IPs`)
    } else {
      failed += 1
...
console.log(failed === 0 ? `ALL V2.5.0 NETWORK SMOKE PASSED (${skipped} reddit skipped)` : `${failed} TOOL(S) FAILED`)
