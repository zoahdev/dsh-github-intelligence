#!/usr/bin/env node
/**
 * Real-network smoke for every tool added in v2.3.0..v2.6.0 (35 tools).
...
async function fetcher(path, signal) {
  const headers = { 'User-Agent': 'dsh-github-intelligence/2.6.0-smoke', Accept: 'application/vnd.github+json' }
...
async function fetcherRaw(path, signal) {
  const response = await fetch(path, {
    headers: { 'User-Agent': 'dsh-github-intelligence/2.6.0-smoke', Accept: 'application/json' },
    signal,
  })
  if (!response.ok) {
...
console.log(failed === 0 ? `ALL V2.6.0 NETWORK SMOKE PASSED (${skipped} reddit skipped)` : `${failed} TOOL(S) FAILED`)
