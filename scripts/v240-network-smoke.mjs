#!/usr/bin/env node
/**
 * Real-network smoke for every tool added in v2.3.0 + v2.4.0 (27 tools).
 * Uses the public APIs of GitHub, GitLab, Gitee, npm, Stack Exchange, Reddit,
 * and dev.to (optionally authenticated GitHub via GH_TOKEN).
 *
...
async function fetcher(path, signal) {
  const headers = { 'User-Agent': 'dsh-github-intelligence/2.4.0-smoke', Accept: 'application/vnd.github+json' }
...
async function fetcherRaw(path, signal) {
  const response = await fetch(path, {
    headers: { 'User-Agent': 'dsh-github-intelligence/2.4.0-smoke', Accept: 'application/json' },
    signal,
  })
  if (!response.ok) {
...
console.log(failed === 0 ? `ALL V2.4.0 NETWORK SMOKE PASSED (${skipped} reddit skipped)` : `${failed} TOOL(S) FAILED`)
